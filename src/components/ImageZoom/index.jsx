...
import {MotivatedImage,SocialLinks} from "../../views/Motivated/MotivatedData"
const ImageZoom = ({closeClick,content}) => {
        ...
        return(
            <ShareBox >
                {SocialLinks.map(items=>{
                    return(
                        <Link key={items.id} onClick={()=>setShare(false)} ><items.Icon/>{items.name}</Link>
                    )
                })
                }
            </ShareBox>
  ...
  return (
    <Container name={screen}>
        <Header>
            <Quantity>{MotivatedImage[currImg].id}/{MotivatedImage.length}</Quantity>
            <Wrapper>
           { click 
        ...
        <ImageWrapper>
        <ImageSlider>
            <LeftArrow onClick={()=>{currImg >0 && setCurrImg(currImg-1)}}/>
            <Images src={MotivatedImage[currImg].image}></Images>
            <RightArrow onClick={()=>{currImg < MotivatedImage.length-1 && setCurrImg(currImg+1)}}/>
        </ImageSlider>
